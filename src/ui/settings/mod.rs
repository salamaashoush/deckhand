...
use gpui::{
  App, AppContext, Context, Entity, FocusHandle, Focusable, Render, SharedString, Styled, Window, div, prelude::*, px,
};
use gpui_component::{
...
impl SettingsDialog {
  pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
    ...
    let container_refresh_input =
      cx.new(|cx| InputState::new(window, cx).default_value(settings.container_refresh_interval.to_string()));

    let stats_refresh_input =
      cx.new(|cx| InputState::new(window, cx).default_value(settings.stats_refresh_interval.to_string()));

    let log_lines_input = cx.new(|cx| InputState::new(window, cx).default_value(settings.max_log_lines.to_string()));

    let font_size_input =
      cx.new(|cx| InputState::new(window, cx).default_value(settings.terminal_font_size.to_string()));
  ...
  fn render_form_row(
    ...
    h_flex()
      ...
      .child(div().w(px(200.)).child(content))
  }
}
